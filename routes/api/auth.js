// This is for users and session (cookies)
...
exports.register = function(server, options, next) {
  server.route([
              ...
              Bcrypt.compare(user.password, userMongo.password, function(err, result) {
                ...
                if (result) {
                  ...
                  db.collection('sessions').insert(newSession, function(err, result) {
                    ...
                    request.yar.set('hapi_hopkong_session', newSession); // CHANGE-ME
    ...
    },
    {
      ...
      handler: function(request, reply) {
        var db = request.server.plugins['hapi-mongodb'].db;
        var session = request.yar.get('hapi_hopkong_session'); // CHANGE-ME
